...
class order_model extends CI_Model
{
    ...
    function getOrders($searchType, $name, $stDate, $enDate, $status)
    {
        $this->db->select('od.id as id, od.value as number, od.userphone as mobile, od.code as price,' .
            ...
            'au.shopid as shop_name, ta.type as type, od.status as status, od.paid_time as ordered_time');
        $this->db->from('tbl_order as od');
        ...
        $this->db->where("(od.ordertype)", '4');
        $this->db->order_by('od.paid_time', 'desc');
    ...
    function getOrderShopIdFromAreaId($shopid, $areaid, $qr_areaid)
    {
        ...
        $this->db->where('targetid', $qr_areaid);
        $query = $this->db->get();
     ...
     * @return number $insert_id : This is last inserted id
     */
    function addPayOrder($value, $phone, $shopid, $qr_areaid, $cost)
    {
        $this->db->select('*');
        $this->db->from('tbl_order');
        $this->db->where('value', $value);
        ...
        $query = $this->db->get();
        $result = $query->result();
        if (count($result) == 0) return NULL; // if ordered data is not exist.
        $OrderInfo = $result['0'];
        $OrderInfo->status = 1;
        $OrderInfo->code = $cost;
        $OrderInfo->authid = $this->getOrderShopIdFromAreaId($shopid, $OrderInfo->areaid, $qr_areaid);

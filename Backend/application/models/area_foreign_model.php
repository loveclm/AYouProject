...
class area_foreign_model extends CI_Model
{
    /**
     * This function is used to get all Tourist Area
     * @return array $result : This is result
     */
    function getAreas($name = '', $continent = 0, $country = '', $status = 0)
    {
        $this->db->select('*');
        $this->db->from('tourist_area');
        if ($continent != '')
            $this->db->where('address', $continent);
        if ($country != '')
            $this->db->where('address_1', $country);
        if ($name != 'all') {
        ...
        $this->db->where('isforeign', 2); //  2-foreign

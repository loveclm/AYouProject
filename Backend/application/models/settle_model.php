...
class settle_model extends CI_Model
{
    ...
    function getAuthCountByShopId($shopid, $date_s, $date_e)
    {
        ...
        $this->db->where("od.ordertype", '4');// auth orders
        $this->db->where("od.status <> '2'");// not paid
        $this->db->where("od.status <> '3'");// canceled
        $this->db->where("od.status <> '0'");// pingtai
        $this->db->where("au.status", '2');// later paid
        $query = $this->db->get();
        $qresult = $query->result();
     ...
     * @return array $result : This is result
     */
    function getAuthSettlePrice($shopid, $month_name)
    {
        $this->db->select('*');

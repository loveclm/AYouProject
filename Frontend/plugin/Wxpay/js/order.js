...
function display_order_data(){
    ...
    for( var i = 0; i < order_List.length; i++){
        ...
        if(parseInt(order_List[i]['state']) != 1){
            tmp_content_html += '<div class="order_footer">';

            switch(parseInt(order_List[i]['state']))
            {
                case 2:
                ...
                case 3:
                case 4:
                    tmp_content_html +='    <div onclick="pay_for_Order('+i+')"><h5>重新购买</h5></div>';
                    break;
            }
            tmp_content_html += '</div>'
        }
...
function cancelOrder(index) {
    sessionStorage.setItem('cancel_order_id', order_List[index].id);

    $('#confirm').show();
    showDialogToCenter('confirm_dialog');
}
...
function purchase_again_Order(index) {
    ...
    var real_cost = cur_order['cost']; //* cur_order['discount_rate'];
    var id = '';
    if( cur_order['order_kind'] == 3)
        id = cur_order['attractionid'];
    else
        id = cur_order['areaid'];

...
function display_order_detail_data(){
    ...
    switch(parseInt(order_detail['state']))
    {
        case 1:  // the case of using state
            ...
            content_html += '   <h5>有效日期&nbsp&nbsp&nbsp  '+ order_detail['expiration_time']+'</h5>';
            content_html += '</div>';
            ...
            if( order_detail['order_kind'] == 1){
                footer_html +='    <div onclick="showMainpage()"><h5>查看线路</h5></div>';
            }else{
        ...
        case 3:   // the case of cancelled state
            ...
            content_html += '</div>';
            //make control button
            footer_html +='    <div onclick="pay_for_Order()"><h5>重新购买</h5></div>';
            break;
